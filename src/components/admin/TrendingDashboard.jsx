...
export default function TrendingDashboard() {
  ...
  if (loading) {
    return (
      <div className="w-full h-64 flex flex-col items-center justify-center glass-card border border-white/10 rounded-2xl bg-[#0A0A0F]/80">
        <div className="w-8 h-8 rounded-full border-4 border-[#6C63FF] border-t-transparent animate-spin mb-4" />
        <p className="text-gray-400 font-dm-sans">Running anomaly detection engine...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="w-full p-8 glass-card border border-red-500/30 rounded-2xl bg-[#0A0A0F]/80">
        <h3 className="text-xl font-syne font-bold text-red-400 mb-2">Error Loading Trending Data</h3>
        <p className="text-gray-400 font-dm-sans">{error}</p>
  ...
  if (!trendingData || trendingData.length === 0) {
    return (
      <div className="w-full p-12 text-center glass-card border border-white/10 rounded-2xl bg-[#0A0A0F]/80">
        <h3 className="text-2xl font-syne font-bold text-white mb-2">No Tracking Data Found</h3>
        <p className="text-gray-400 font-dm-sans max-w-md mx-auto">
          The engine requires view data. Ensure your tracking schema is applied and views are being logged in Supabase.
        </p>
        <button 
          onClick={handleRecalculate}
          className="mt-6 px-6 py-3 bg-[#6C63FF]/20 border border-[#6C63FF]/30 text-[#6C63FF] font-medium rounded-xl hover:bg-[#6C63FF]/30 transition-colors"
        >
          Check Again
        </button>
      </div>
  ...
  return (
    <div className="w-full space-y-8 font-dm-sans">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button
          onClick={handleRecalculate}
          className="px-6 py-3 bg-gradient-to-r from-[#6C63FF] to-[#00D4AA] text-white font-bold rounded-xl hover:opacity-90 transition-opacity shadow-[0_0_15px_rgba(108,99,255,0.3)] flex items-center gap-2"
        >
      ...
      {/* Top 10 Bar Chart */}
      <div className="glass-card border border-white/10 rounded-2xl bg-[#0A0A0F]/80 p-6 md:p-8">
            ...
            <BarChart data={top10Volume} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Bar dataKey="views" radius={[4, 4, 0, 0]}>
                {top10Volume.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={index < 3 ? '#00D4AA' : '#6C63FF'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {trendingData.map((tool, index) => (
            <motion.div
              ...
              key={tool.id}
              className="glass-card border border-white/5 rounded-xl p-5 bg-white/[0.02] hover:bg-white/[0.04] transition-colors relative overflow-hidden"
            >
              {/* Status Color Glow */}
              <div 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Z-Score</p>
              ...
              <div className="mt-4 pt-4 border-t border-white/5 flex justify-between items-center text-sm">
                <span className="text-gray-400">7-Day Velocity:</span>
